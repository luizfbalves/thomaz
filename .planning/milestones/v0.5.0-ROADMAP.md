# Roadmap: thomaz — v0.5.0 Theme Extraction (archived)

> **Status: partially shipped 2026-06-05.** Phases 1-2 (the on-device extraction
> engine) shipped and merged to main as app version **0.5.0** (tag `v0.5.0`).
> Phases 3-4 (theme-UI integration + optional forwarder) were **planned but NOT
> built** — no `extract_state_store`, no theme-browser extraction row — and are
> **carried forward** to a future milestone. Originally tracked as "v0.4 Extração
> de Temas" on the `workspace/temas` branch.
...
| 2. Full Extraction Engine | 4/4 | Complete (on-hardware verify deferred) | 2026-06-05 |
| 3. Theme UI Integration | 0/3 | Carried forward (not built) | - |
| 4. Forwarder (Optional) | 0/1 | Carried forward (not built) | - |
