...
void nca_save_pfs0_section(nca_section_ctx_t* ctx) {
	if (ctx->superblock_hash_validity == VALIDITY_VALID && ctx->pfs0_ctx.header->magic == MAGIC_PFS0) {
		...
		if (ctx->pfs0_ctx.is_exefs && ctx->tool_ctx->settings.extraction_exefs && ctx->tool_ctx->settings.extraction_file_stream_cb)
		{
			uint64_t size = 0;
			uint8_t* data = nca_copy_pfs0_file(ctx, "main", &size);

			ctx->tool_ctx->settings.extraction_file_stream_cb(
				ctx->tool_ctx->settings.extra_context, "main", data, size);
		}
...
static void read_nca_entry(nca_section_ctx_t* ctx, romfs_fentry_t* entry, uint64_t ofs, unsigned char** out_data, size_t* out_length)
{
	...
	memset(buf, 0xCC, entry->size); // Debug

	nca_section_fseek(ctx, ofs);
	if (nca_section_fread(ctx, buf, entry->size) != entry->size) {
...
static int nca_visit_romfs_file(nca_section_ctx_t* ctx, uint32_t file_offset, filepath_t* dir_path) {
	...
	do {
		...
		if (entry->name_size)
		{
			if (entry->name_size >= MAX_PATH) {
				fprintf(stderr, "Entry name is too long!\n");
				exit(EXIT_FAILURE);
			}

			// the last entry->name is not null-terminated we can't pass it with %s without copying it to a temporary buffer first
			memcpy(tmp_path, entry->name, entry->name_size);
			tmp_path[entry->name_size] = '\0';
			filepath_append_n(cur_path, entry->name_size, "%s", tmp_path);
		}
		...
		if ((ctx->tool_ctx->action & ACTION_ONLYUPDATEDROMFS) == 0 || nca_is_romfs_file_updated(ctx, phys_offset, entry->size)) {
			if ((ctx->tool_ctx->action & ACTION_LISTROMFS) == 0) {
				...
				if (ctx->tool_ctx->settings.romfs_filter)
					should_extract = ctx->tool_ctx->settings.romfs_filter(ctx->tool_ctx->settings.extra_context, cur_path->char_path);
				...
				else
				{
					...
					if (ctx->tool_ctx->settings.extraction_file_stream_cb)
					{
						size_t length = 0;
						unsigned char* data = NULL;
						read_nca_entry(ctx, entry, phys_offset, &data, &length);
						ctx->tool_ctx->settings.extraction_file_stream_cb(
							ctx->tool_ctx->settings.extra_context, cur_path->char_path, data, length);
					}
					else
					{
	...
	if (ctx->superblock_hash_validity == VALIDITY_VALID) {
		if (ctx->romfs_ctx.header.header_size == ROMFS_HEADER_SIZE) {
			if (ctx->tool_ctx->action & ACTION_LISTROMFS) {
				...
				{
					filepath_t no_path;
					filepath_init(&no_path);
					filepath_set(&no_path, "");
					nca_visit_romfs_dir(ctx, 0, &no_path);
				}
			}
